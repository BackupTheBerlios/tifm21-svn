...
struct tifmxx_sock_data
{
	...
	int                sock_status; // current socket status
};
...
tifmxx_scsi_queue(struct scsi_cmnd *srb, void (*done)(struct scsi_cmnd*))
{
	...
	tasklet_schedule(&fm->work_thread);
	spin_unlock_irqrestore(fm->lock, f1);  
	return 0;
}
...
{	
	...
	tasklet_init(&fm->work_thread, tifmxx_work_thread, (unsigned long)fm);
...
tifmxx_remove (struct pci_dev *pdev)
{
	...
	spin_lock_irqsave(fm->lock, f1);
	tasklet_kill(&fm->work_thread); // I hope this is needed and enough
	//! reset hardware	
	//! kick all pending scsi commands
	//! remove all devices
	spin_unlock_irqrestore(fm->lock, f1);
